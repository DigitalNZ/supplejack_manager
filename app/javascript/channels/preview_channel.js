  ...
  return consumer.subscriptions.create(
    { channel: "PreviewChannel", id: id },
    {
      connected() {
        ...
        initCodeMirrorOnTabsClick(
          "#harvested-attributes-label",
      ...
      received(data) {
        ...
        if (data.field_errors != null && data.field_errors != "{}") {
          ...
          $("#field-errors").show();
          $("#field-errors textarea").val(
            JSON.stringify(JSON.parse(data.field_errors), null, 4)
          );
          $("#field-errors .CodeMirror").remove();
          window.initCodeMirror(
            document.querySelector("#field-errors textarea")
          );
        }
        ...
        if (data.harvest_failure) {
          ...
          $("#harvest-failure h4").html(
            `Harvest failure: ${failure.exception_class} "${failure.message}"`
          );
        ...
        if (data.enrichment_failures) {
          ...
          $.each(failures, function (_, failure) {
            ...
            <details class="details details--warning">
              <summary class="details__summary"><h4>Enrichment "${
                failure.enrichment_name
              }" failed: ${failure.exception_class} "${failure.message}"</h4></summary>
              <div class="details__content">${failure.backtrace.join(
                "<br />"
              )}</div>
            </details>
          `);
